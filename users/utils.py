...
def author_group():
    user_permissions = [p for p in Permission.objects.filter(Q(content_type__app_label__in=['users'])
                                                        & Q(codename__icontains='User'))]
    magazine_permissions = [p for p in Permission.objects.filter(Q(content_type__app_label__in=['magazines'])
